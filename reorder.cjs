...
let content = fs.readFileSync('src/components/FAQModal.jsx', 'utf8');
...
if (faqsStart === -1 || faqsEnd === -1) {
  console.error('Could not find faqs array');
  process.exit(1);
}

const faqsContent = content.substring(faqsStart, faqsEnd);

// Use a simple replacement strategy
const newFaqsContent = `const faqs = [
    {
...
const newContent = content.substring(0, faqsStart) + newFaqsContent + content.substring(faqsEnd);
fs.writeFileSync('src/components/FAQModal.jsx', newContent, 'utf8');
console.log('Successfully reordered FAQ!');
